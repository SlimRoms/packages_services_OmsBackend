            ...
            try {
                ...
                if (overlays != null) {
                    ...
                    for (Overlay overlay : overlays.overlays) {
                        ...
                        PackageInfo appInfo = getPackageManager().getPackageInfo(overlay.targetPackage, 0);
                        generateManifest(theme, overlay, overlayFolder.getAbsolutePath(), appInfo.versionCode);
                ...
                if (overlays != null) {
                    for (Overlay overlay : overlays.overlays) {
                        if (overlay.checked) {
                            ...
                            try {
                                FileUtils.copyFile(new File(overlay.tag, "fonts.xml"),
                                        new File(THEME_FONT_PATH, "fonts.xml"));
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                            final Gson gson = new GsonBuilder().create();
